...
const TechIcon = ({ children, style }: { children: React.ReactNode, style: React.CSSProperties }) => (
  <div className="absolute text-muted-foreground/40" style={style}>
    {children}
  </div>
);
  ...
  const galleryItems = [
    {
      ...
      desc: "A passionate developer, always exploring new horizons in technology and design.",
    },
    {
      img: "https://placehold.co/600x400.png",
      dataAiHint: "modern workspace",
      desc: "My creative space where ideas come to life. Clean, minimal, and focused.",
    },
    {
      img: "https://placehold.co/600x400.png",
      dataAiHint: "laptop code",
      desc: "When not coding professionally, I love working on personal projects that challenge me.",
    },
    {
      img: "https://placehold.co/600x400.png",
      dataAiHint: "server room",
      desc: "Building and deploying scalable infrastructure is one of my passions.",
    },
     {
      img: "https://placehold.co/600x400.png",
      dataAiHint: "conference talk",
      desc: "Sharing knowledge and speaking at tech conferences.",
    },
  ];
        ...
        <section className="w-full h-screen flex items-center bg-background relative overflow-hidden">
          <div className="container px-4 md:px-6 grid md:grid-cols-2 gap-8 items-center z-10">
            ...
            <div className="relative flex flex-col space-y-6 text-left h-full justify-center">
                 <div className="absolute inset-0 z-0 [mask-image:radial-gradient(ellipse_at_center,white_20%,transparent_70%)]">
                    {isMounted && skills.map((skill, index) => (
                        <TechIcon key={index} style={iconStyles[index]}>
                            {skill.icon}
                        </TechIcon>
                    ))}
                </div>
